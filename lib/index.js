...
var findOne = function (html, identificator, selectors) {
    var afterIdentificator = findUntil(identificator, html);
    var excess = afterIdentificator;
    var result = [];
...
var findAll = function (html, identificator, selectors) {
    ...
    var lhtml = html;
    for (var i = 0; true; i++) {
        var _a = findOne(lhtml, identificator, selectors), result = _a.result, ehtml = _a.excessHTML;
        if (result.length == 0)
            return search;
        search.push(result);
        lhtml = ehtml;
